...
bool events::out::generictext(std::string packet) {
    ...
    if (var.get(0).m_key == "action" && var.get(0).m_value == "input") {
        ...
        if (find_command(chat, "name ")) { //ghetto solution, but too lazy to make a framework for commands.
            ...
            gt::ghost = !gt::ghost;
            if (gt::ghost)
                gt::send_log("`3Ghost is now enabled.");
            else
                gt::send_log("`3Ghost is now disabled.");
            return true;
        } else if (find_command(chat, "country ")) {
            std::string cy = chat.substr(9);
            gt::flag = cy;
            gt::send_log("`3your country set to " + cy + ", (Relog to game to change it successfully!)");
            return true;
        }
        else if (find_command(chat, "fd")) {
            fastdrop = !fastdrop;
            if (fastdrop)
                gt::send_log("`3Fast Drop is now enabled.");
            else
                gt::send_log("`3Fast Drop is now disabled.");
            return true;
        }
        else if (find_command(chat, "ft")) {
            fasttrash = !fasttrash;
            if (fasttrash)
                gt::send_log("`3Fast Trash is now enabled.");
            else
                gt::send_log("`3Fast Trash is now disabled.");
            return true;
        }        
        else if (find_command(chat, "wrenchmsg")) {
            wrenchmsg = !wrenchmsg;
            if (wrenchmsg)
                gt::send_log("`3wrenchmsg is now enabled.");
            else
                gt::send_log("`3wrenchmsg is now disabled.");
            return true;
         } 
             else if (find_command(chat, "automsg")) {
            automessage = !automessage;
            if (automessage)
                gt::send_log("`3automsg is now enabled.");
            else
                gt::send_log("`3automsg is now disabled.");
            return true;
          } 
             else if (find_command(chat, "autopull")) {
            autopull = !autopull;
            if (autopull)
                gt::send_log("`3autopull is now enabled.");
            else
                gt::send_log("`3autopull is now disabled.");
            return true;
         } 
            else if (find_command(chat, "pullauto")) {
            pullauto = !pullauto;
            if (pullauto)
                gt::send_log("`3Pull Auto is now enabled.");
            else
                gt::send_log("`3Pull Auto is now disabled.");
            return true;
         }                  
        else if (find_command(chat, "wrenchspam")) {
            wrenchspam = !wrenchspam;
            if (wrenchspam)
                gt::send_log("`3wrenchspam is now enabled.");
            else
                gt::send_log("`3wrenchspam is now disabled.");
            return true;
        }  
       ...
       return true;
            ...
            mode = chat.substr(10);
            gt::send_log("`3Wrench mode set to " + mode);
            return true;        
        }
        else if (find_command(chat, "wrenchmode")) {
            wrench = !wrench;
            if (wrench)
                gt::send_log("`3Wrench mode is on.");
            else
                gt::send_log("`3Wrench mode is off.");
            return true;
         }
            ...
            for (auto& player : g_server->m_world.players) {
                ...
                if (name_2.find(name) == 0) {
                    gt::send_log("`3Teleporting to " + player.name);
           ...
           std::string msgtext = "              Message from FakeModz YT";
            ...
            paket =
               ...
               "\nadd_image_button|banner|interface/large/special_event.rttex|bannerlayout|||"
                "\nadd_spacer|small"
                "\nadd_textbox|`3Proxy Made By Erza gt #erzagt|left|2480|"
           ...
           // gt::send_log(
            ...
            //    "`3/warp [world name] (warping world without SSUP), /skin [Id] (change skin colours), /wrenchmode (for wrench pull, kick, pull, ban, trade)"
           //     "`3/ft (fast trash), /fd (fast drop), /setcountry (bug), /wrenchset (for set wrenchmode : pull,kick,ban,trade,add friend),/msgall (bug), /pinfo"
            //    "`3/wrenchmsg (Auto Msg Wrench People), /setmsg (for set message text)");
           std::string paket1;
            paket1 =
                "\nadd_label_with_icon|big|Erza Proxy Commands|left|20|"
                "\nadd_image_button|banner|interface/large/news_banner.rttex|bannerlayout|||"
                "\nadd_spacer|small"
                "\nadd_textbox|`3/tp [name] (teleports to a player in the world)|left|2480|"
                "\nadd_textbox|`3/ghost (toggles ghost, you wont move for others when its enabled)|left|2480|"
                "\nadd_textbox|`3/uid [name] (resolves name to uid)|left|2480|"
                "\nadd_textbox|`3/flag [id] (sets flag to item id)|left|2480|"
                "\nadd_textbox|`3/name [name] (Change Name Visual)|left|2480|"
                "\nadd_textbox|`3/banall (World Ban All People in world)|left|2480|"
                "\nadd_textbox|`3/killall (Kick all People in world)|left|2480|"
                "\nadd_textbox|`3/tradeall (trade all people in the world|left|2480|"
                "\nadd_textbox|`3/warp [world name] (warping world without SSUP)|left|2480|"
                "\nadd_textbox|`3/skin [Id] (change skin colours)|left|2480|"
                "\nadd_textbox|`3/wrenchmode (wrench modefor wrench pull, kick, pull, ban, trade, add)|left|2480|"
                "\nadd_textbox|`3/wrenchset (for set wrenchmode : pull,kick,ban,trade,add friend)|left|2480|"
                "\nadd_textbox|`3/ft (fast trash) |left|2480|"
                "\nadd_textbox|`3/fd (fast drop) |left|2480|"
                "\nadd_textbox|`3/wrenchmsg (Auto Msg when wrench people) |left|2480|"
                "\nadd_textbox|`3/setmsg (Costum Text for Wrenchmsg and wrenchspam) |left|2480|"
                "\nadd_textbox|`3/country (/countrylist for check list)|left|2480|"
                "\nadd_textbox|`3/msgall (not really worked because spam detected) |left|2480|"
                "\nadd_textbox|`3/wrenchspam (wrench spam like wrench msg do/setspam for set text) |left|2480|"
                "\nadd_textbox|`3/automsg (auto msg when people enter world) |left|2480|"
                "\nadd_textbox|`3/door (teleport to id door (you must know the id door)) |left|2480|"
                "\nadd_textbox|`3/pinfo (Proxy information) |left|2480|"
                "\nadd_textbox|`3/countrylist (List Country For /country) |left|2480|"
                "\nadd_textbox|`3/autopull (auto pull when people enter world) |left|2480|"
                "\nadd_textbox|`3/pullauto (auto pull for casino hoster) |left|2480|"
                "\nadd_spacer|small|\n\nadd_url_button||`$Discord``|NOFLAGS|https://discord.gg/FFB2WGmNjj|Open link?|0|0|"
                "\nadd_quick_exit|"
                "\nend_dialog|end|Cancel|Okay|";
            variantlist_t liste{ "OnDialogRequest" };
            liste[1] = paket1;
...
bool events::in::variantlist(gameupdatepacket_t* packet) {
    ...
    switch (hs::hash32(func.c_str())) {
        ...
        case fnv32("OnConsoleMessage"): {
            varlist[1] = "`0[`^ErzaProxy`0]`` " + varlist[1].get_string();
...
//std::transform(console.begin(),console.end(),console.begin(),::tolower);
    ...
    if (content.find("embed_data|netID") !=-1) {
     if(content.find("Add as friend") !=-1) {
        //std::string yourmsg = "MESSAGE ME ON DISCORD Erza GT";
        ...
        titit.erase(titit.begin() + titit.find(" (`3"), titit.end());
...
if (wrenchspam == true) {
    if (content.find("embed_data|netID") !=-1) {
     if(content.find("Add as friend") !=-1) {
        ...
        //titit1.erase(titit1.begin() + titit1.find(" (`3"), titit1.end());
